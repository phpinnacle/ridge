...
class ContentHeaderFrame extends AbstractFrame
{
    ...
    public function pack(): Buffer
    {
        ...
        if($flags & self::FLAG_CONTENT_TYPE)
        {
            $buffer->appendString((string) $this->contentType);
        }

        if($flags & self::FLAG_CONTENT_ENCODING)
        {
            $buffer->appendString((string) $this->contentEncoding);
        }
        ...
        if($flags & self::FLAG_DELIVERY_MODE)
        {
            $buffer->appendUint8((int) $this->deliveryMode);
        }

        if($flags & self::FLAG_PRIORITY)
        {
            $buffer->appendUint8((int) $this->priority);
        }

        if($flags & self::FLAG_CORRELATION_ID)
        {
            $buffer->appendString((string) $this->correlationId);
        }

        if($flags & self::FLAG_REPLY_TO)
        {
            $buffer->appendString((string) $this->replyTo);
        }

        if($flags & self::FLAG_EXPIRATION)
        {
            $buffer->appendString((string) $this->expiration);
        }

        if($flags & self::FLAG_MESSAGE_ID)
        {
            $buffer->appendString((string) $this->messageId);
        }
        ...
        if($flags & self::FLAG_TYPE)
        {
            $buffer->appendString((string) $this->typeHeader);
        }

        if($flags & self::FLAG_USER_ID)
        {
            $buffer->appendString((string) $this->userId);
        }

        if($flags & self::FLAG_APP_ID)
        {
            $buffer->appendString((string) $this->appId);
        }

        if($flags & self::FLAG_CLUSTER_ID)
        {
            $buffer->appendString((string) $this->clusterId);
        }
